...
#define RAW_DEC_LEN 3   /* max. possible length of a byte represented in decimal */                                
                        /* raw, as in they aren't formatted */

#define DEC_LEN (RAW_DEC_LEN + SPACE_LEN)                               
#define DEC_GROUP_SIZE 4.0                                  /* the number offormatted dec values in one group */
#define DEC_GROUP_LEN (DEC_LEN*DEC_GROUP_SIZE + SPACE_LEN)  /* how many characters a formatted dec group takes up */
...
void print_raw_decimal(unsigned int value) {

    printf("%0*d", RAW_DEC_LEN, value);
}
...
void print_header(size_t current_line, double lines, double dec_per_line) {
        size_t completion_pct = (current_line + 1)/lines*100;
        size_t completion_pct_padding = COMPLETION_PCT_LEN - PCT_SIGN_LEN;

        size_t dec_printed = dec_per_line*(double)current_line;
        size_t dec_printed_padding = int_len(dec_per_line*lines);


        printf("%0*u%% %0*u| ", completion_pct_padding, completion_pct, dec_printed_padding, dec_printed);
...
int main(int argc, char* argv[]) {
    ...
    double dec_groups_per_line = floor((payload_len - NEW_LINE_LEN)/DEC_GROUP_LEN);
    double dec_per_line = floor(dec_groups_per_line*DEC_GROUP_SIZE);

    size_t lines = (size_t)ceil((double)FILE_SIZE/dec_per_line);
    ...
    unsigned int dec; // has to be int to be able to check for EOF
    for (size_t line = 0; line < lines; line++)
    {
        print_header(line, lines, dec_per_line);

        for (size_t _ = 0; _ < dec_groups_per_line; _++)
        {

            for (size_t _ = 0; _ < DEC_GROUP_SIZE && (dec = fgetc(file)) != EOF; _++) 
            {
                print_raw_decimal(dec);
                printf(" ");
            }
            printf(" ");
